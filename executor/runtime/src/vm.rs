...
// TODO: Replace lmdb with kv_storage traits from main codebase

pub struct VmState<'a> {
    registers: IntMap<u64, RefCell<Vec<u8>>>,
    db_handle: &'a Environment,
    db_ptr: Database,
    db_acid_txn: Option<RwTransaction<'a>>,
...
impl<'a> VmState<'a> {
    pub fn new(db_handle: &'a Environment, db_ptr: Database) -> VmState<'a> {
        VmState {
...
fn get_memory(caller: &mut Caller<'_, VmState>) -> wasmtime::Result<Memory> {
...
pub fn tic(mut caller: Caller<'_, VmState>) {
    caller.data_mut().last_timer = Some(Instant::now());
}

pub fn toc(mut caller: Caller<'_, VmState>) {
...
pub fn register_len(caller: Caller<'_, VmState>, register_id: u64) -> u64 {
...
pub fn storage_read(
    ...
    // Write to register
    // Why not directly write into wasm memory ?
    // -> Is this because of unknown length ? Reading from register is a two step process,
    //    so I think we need to "buffer" the value into a register, before we can use it.
...
pub fn storage_has_key(
    ...
    let found_key = match result {
        Ok(_) => 1,
        Err(e) => match e {
            lmdb::Error::NotFound => 0,
            _ => return Err(wasmtime::Error::msg(format!("failed to read from db: {e}"))),
        },
    };
...
// NOTE: If there are two acid transactions, this is a caller error, and not a runtime error.
pub fn storage_begin_acid_txn(mut caller: Caller<'_, VmState>) -> wasmtime::Result<u64> {
    caller.data_mut().begin_acid_txn()
}

// NOTE: If there are two acid transactions, this is a caller error, and not a runtime error.
pub fn storage_commit_acid_txn(mut caller: Caller<'_, VmState>) -> wasmtime::Result<u64> {
