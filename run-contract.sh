...
if [[ $WASMTIME_BACKTRAC_DETAILS == "1" ]]; then
  ...
  cargo run -p runner -- --contract $DIR/target/wasm32-unknown-unknown/debug/$CONTRACT.wasm --db $DB_PATH --action $ACTION
else
  # Build and execute release
  cargo build -p $PACKAGE --target=wasm32-unknown-unknown --release || exit 3 
  cargo run -p runner -- --contract $DIR/target/wasm32-unknown-unknown/release/$CONTRACT.wasm --db $DB_PATH --action $ACTION
fi
