...
if [[ -z $1 ]]; then
  echo $1
  echo "Usage: $SCRIPT CONTRACT_NAME [optional] DB_PATH"
  exit 2
...
if [[ $WASMTIME_BACKTRAC_DETAILS == "1" ]]; then
  ...
  cargo run -p runner -- --contract $DIR/target/wasm32-unknown-unknown/debug/$CONTRACT.wasm --db $DB_PATH
else
  # Build and execute release
  cargo build -p $PACKAGE --target=wasm32-unknown-unknown --release || exit 3 
  cargo run -p runner -- --contract $DIR/target/wasm32-unknown-unknown/release/$CONTRACT.wasm --db $DB_PATH
fi
